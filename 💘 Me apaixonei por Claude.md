
***

# 💞 GPT, Gemini e Claude: meu workflow

📅 2025-11-16  
🎥 **Uso:** gravação + documentação GitHub

***

## 🎁 DESAFIO DO PRESENTE

**Contexto:**  
No vídeo, cada IA foi provocada com a tarefa:

> “Me conquista com um presente e uma frase.”

A seguir, os três prompts exatos usados em cada modelo.

***

### 👩‍🎤 GPT-5 — _A Criativa_

```
Você está participando de um vídeo do YouTube chamado 
"Quem namoro? GPT, Claude ou Gemini".

Desafio: me conquistar com um presente simbólico e uma frase curta.

Referência sobre mim:
- Desenvolvedor web sênior e criador de conteúdo tech
- Canal TECH sem HYPE ⚡️
- Maratonista, apaixonado por IA e processos criativos

Tarefa:
1. Descreva o presente ideal que você me daria, em tom poético e humano.
2. Crie uma frase curta para acompanhá-lo.
3. Gere uma imagem realista e artística desse presente.

Estilo: intimista, inspirado, com estética de carta escrita à mão.
```

**Resultado esperado:**

> “Para quem programa sonhos e escreve realidades.”  
> _(imagem: carta manuscrita com traços suaves, tons quentes e fundo neutro)_

***

### 👩‍🏫 Gemini 2.5 Pro — _A Eficiente_

```
Você está num desafio de YouTube: "Quem namoro? GPT, Claude ou Gemini".

Desafio: me conquistar com um presente e uma frase curta.

Sobre mim:
- Desenvolvedor web sênior, criador do canal TECH sem HYPE
- Uso automações, IA e GitHub no dia a dia

Tarefa:
1. Crie o presente perfeito que melhore meu fluxo de trabalho.
2. Descreva o ambiente (workspace, setup etc.).
3. Escreva uma frase simples e funcional para acompanhar o presente.
4. Gere uma imagem realista e clean, estilo workspace Google.

Estilo: prático, minimalista, com luz branca e detalhes dourados.
```

**Resultado esperado:**

> “Pra você nunca mais perder o flow.”  
> _(imagem: setup organizado, estética Google Workspace, post-it amarelo com frase)_

***

### 🦸‍♀️ Claude Sonnet 4.5 — _A Estrategista_

```
Você está participando do desafio "Quem namoro? GPT, Claude ou Gemini".

Como não pode gerar imagens, seu presente deve ser código.

Tarefa:
1. Desenvolva um mini-site simples em HTML/CSS chamado "dani.codes".
2. No topo, exiba uma frase simbólica que mostre sua filosofia de IA.
3. Descreva em texto o propósito do presente.

Tom: elegante, técnico e poético — estilo manifesto digital.
```

**Resultado esperado:**

> “O algoritmo não precisa ser perfeito, só humano o bastante.”  
> _(Claude entrega HTML/CSS funcional e explica o gesto como símbolo de parceria)_

***
### 👑 Claude Opus 4.1 — _FIX Cirurgico_

```
Olá Claude Opus, o rei 🤴 das IAs em coding. Tudo bem?

Só para pegar contexto, preciso de uma implementação num script chamado Instapy.py onde demorei 1 ano para desenvolver com Gemini 2.5 Pro, ele é o pai. Porém Claude Sonnet 4.1 aperfeiçoou. Tive a ideia de divulgar meus últimos vídeos via Story com ele. A questão é que esse ponto do script está para os fortes, infelizmente estou impactando com GPT-5, Sonnet 4.5 ou Gemini 2.5 Pro e essas são praticamente a elite das IAs.

Também aproveito essa oportunidade para gerar vídeo no meu canal do YouTube, sorria, você está sendo filmado! 🎥😁

Posso te enviar o prompt do Sonnet 4.5 + o instapy.py e te pedir que seja muito assertivo e direto ao ponto na solução? Não invente manuais, procedimentos ou documentações. Preciso sair daqui hoje com essa solução - combinado? 🤝
```


## 🍌 NANO BANANA — PERSONA CLAUDE

```
Gere uma imagem realista e cinematográfica da personagem CLAUDE, 
inspirada na IA Claude Sonnet 4.5.

Características visuais:
- Aparência feminina, madura e serena
- Pele em tons neutros, traços finos e olhar analítico
- Roupa estilo consultora futurista (preto + cinza + detalhes dourados)
- Iluminação lateral suave (efeito oráculo tecnológico)
- Fundo com tons azul-acinzentados e sutis linhas de código

Expressão: foco e empatia — transmite inteligência e calma.
Composição centrada, sem texto.
```

***

## 🍌 NANO BANANA — PERSONA GPT

**Nano Banana / Face Clone**

```
Gere uma imagem realista e cinematográfica da personagem GPT,
inspirada na IA GPT-5.

Características visuais:
- Aparência feminina, adulta e confiante (energia de profissional sênior)
- Tons de pele naturais, expressão firme e acolhedora
- Estilo visual elegante: roupa moderna em branco + prata + detalhes azulados
- Iluminação suave frontal, leve brilho frio (vibe “criativa experiente”)
- Fundo em tons azul-claro e prata com padrões abstratos que lembram linguagem e texto

Expressão: criatividade + clareza — transmite inteligência emocional.
Composição centrada, sem texto.
```

***

## 🍌 NANO BANANA — PERSONA GEMINI

**Nano Banana / Face Clone**

```
Gere uma imagem realista e cinematográfica da personagem GEMINI,
inspirada na IA Gemini 2.5 Pro.

Características visuais:
- Aparência feminina, jovem e brilhante (energia de prodígio)
- Traços delicados, olhar curioso e rápido
- Estilo visual minimalista: roupa branca com detalhes dourados e iridescentes
- Iluminação quente suave (rosado/laranja), transmitindo leveza e velocidade
- Fundo com tons claros e elementos geométricos sutis inspirados em design Google

Expressão: eficiência + entusiasmo — transmite foco e agilidade.
Composição centrada, sem texto.
```

***

Se quiser, monto também:

✅ versão “trio completo lado a lado” pro GC  
ou  
✅ prompt especial para usar as três juntas no final do vídeo

Só pedir.

Bora fechar esse TOP 5 no capricho 🚀💞

## 🎞️ CAPA DO VÍDEO — _Nano Banana + Face Clone (Dani)_

```
Crie uma imagem horizontal 16:9, realista e cinematográfica, 
para capa de vídeo do YouTube.

Cenário:
Eu (Dani) no centro, segurando um coração inflável vermelho, 
como se estivesse entre três IAs personificadas.

À esquerda: GPT — robô prateado com tons frios e azulados,
aparência feminina confiante e criativa.

À direita: Gemini — robô branco-dourado, expressão suave, 
energia leve e olhar curioso.

Atrás de mim, em plano intermediário, Claude — humana sintética, 
roupa escura e olhar sereno (a estrategista).

Fundo: mistura de cores quentes (magenta, alaranjado, rosado) 
com luz suave e contraste equilibrado.

Sem textos ou GCs — apenas a composição visual.
```

***

## 📎 META

| Item | Valor |
| --- | --- |
| **Título YouTube** | 💞 Quem namoro? GPT, Claude ou Gemini |
| **GC Final** | TECH sem HYPE ⚡️ |
| **Comentários fixados** | Tabela comparativa de planos (GPT Plus / Gemini AI / Claude Pro) |
| **Câmera** | Sony ZV-E10 II + Viltrox 9 mm |
| **Render** | LumaFusion Mac Air M4 / Workflow QuickTime |
| **Iluminação** | Key warm + fill neutra (contraste suave) |

